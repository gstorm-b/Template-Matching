...
void TmplMatching::DrawDashLine (Mat& matDraw, Point ptStart, Point ptEnd, Scalar color1, Scalar color2) {
...
void TmplMatching::DrawMarkCross (Mat& matDraw, int iX, int iY, int iLength, Scalar color, int iThickness) {
  if (matDraw.empty ())
    return;
  Point ptC (iX, iY);
...
void TmplMatching::LearnPattern() {
  // UpdateData (1);;
  m_TemplData.clear ();
...
int TmplMatching::GetTopLayer (Mat* matTempl, int iMinDstLength) {
  int iTopLayer = 0;
...
bool TmplMatching::Match () {
  ...
  qDebug() << "Raw object founded:" << vecMatchParameter.size();
  ...
  qDebug() << "Excution time:" << int(clock () - d1);
  ...
  for (int i = 0 ; i < (int)m_vecSingleTargetData.size (); i++)
  {
    ...
    qDebug() << sstm.dMatchScore << sstm.dMatchedAngle << sstm.ptCenter.x << sstm.ptCenter.y;
  }
  // m_strTotalNum.Format (L"%d", (int)m_vecSingleTargetData.size());
  qDebug() << "Total detect:" << (int)m_vecSingleTargetData.size();
  // UpdateData (false);
  m_bShowResult = true;
...
bool TmplMatching::SubPixEsimation (vector<s_MatchParameter>* vec, double* dNewX, double* dNewY, double* dNewAngle, double dAngleStep, int iMaxScoreIndex)
{
...
void TmplMatching::OutputRoi (s_SingleTargetMatch sstm) {
  if (!m_ckOutputROI)
    return;
  Rect rect (sstm.ptLT, sstm.ptRB);
  for (int i = 1 ; i < 50 ; i++)
  {
    String strName = format ("roi%d.bmp", i);
...
void TmplMatching::MatchTemplate (cv::Mat& matSrc, s_TemplData* pTemplData, cv::Mat& matResult, int iLayer, bool bUseSIMD)
{
...
void TmplMatching::GetRotatedROI (Mat& matSrc, Size size, Point2f ptLT, double dAngle, Mat& matROI)
{
...
void TmplMatching::CCOEFF_Denominator (cv::Mat& matSrc, s_TemplData* pTemplData, cv::Mat& matResult, int iLayer)
{
  if (pTemplData->vecResultEqual1[iLayer])
  {
    matResult = Scalar::all (1);
    qDebug() << "Result equal 1";
    return;
  }
  ...
  // integral (matSrc, sum, sqsum, CV_8UC);
...
Size TmplMatching::GetBestRotationSize (Size sizeSrc, Size sizeDst, double dRAngle)
{
...
Point2f TmplMatching::ptRotatePt2f (Point2f ptInput, Point2f ptOrg, double dAngle)
{
...
void TmplMatching::FilterWithScore (vector<s_MatchParameter>* vec, double dScore)
{
...
void TmplMatching::FilterWithRotatedRect (vector<s_MatchParameter>* vec, int iMethod, double dMaxOverLap)
{
  ...
  for (int i = 0; i < iMatchSize - 1; i++)
  {
    ...
    for (int j = i + 1; j < iMatchSize; j++)
    {
      ...
      {
        // qDebug() << "Intersect none."
        //          << rect1.center.x << rect1.center.y
        //          << rect1.center.x << rect1.center.y;
        continue;
      }
      else if (iInterSecType == INTERSECT_FULL) //一個矩形包覆另一個
      {
        // qDebug() << "Intersect full.";
        int iDeleteIndex;
...
Point TmplMatching::GetNextMaxLoc (Mat & matResult, Point ptMaxLoc, Size sizeTemplate, double& dMaxValue, double dMaxOverlap) {
  //比對到的區域完全不重疊 : +-一個樣板寬高
...
Point TmplMatching::GetNextMaxLoc (Mat & matResult, Point ptMaxLoc, Size sizeTemplate, double & dMaxValue, double dMaxOverlap, s_BlockMax & blockMax) {
  //比對到的區域需考慮重疊比例
...
void TmplMatching::SortPtWithCenter (vector<Point2f>& vecSort)
{
...
void TmplMatching::RefreshSrcView () {
  m_matResult = m_matSrc.clone();
  ...
  for (int i = 0; i < iSize; i++)
  {
    ...
    DrawDashLine (m_matResult, ptLT, ptLB);
    DrawDashLine (m_matResult, ptLB, ptRB);
    DrawDashLine (m_matResult, ptRB, ptRT);
    DrawDashLine (m_matResult, ptRT, ptLT);
    ...
    line (m_matResult, ptLT, ptLT + ptDis1 / 2, Scalar(0, 255, 0), 1, CV_AA);
    line (m_matResult, ptLT, ptLT + ptDis2 / 2, Scalar(0, 255, 0), 1, CV_AA);
    line (m_matResult, ptRT, ptRT + ptDis1 / 2, Scalar(0, 255, 0), 1, CV_AA);
    line (m_matResult, ptRT, ptRT - ptDis2 / 2, Scalar(0, 255, 0), 1, CV_AA);
    line (m_matResult, ptRB, ptRB - ptDis1 / 2, Scalar(0, 255, 0), 1, CV_AA);
    line (m_matResult, ptRB, ptRB - ptDis2 / 2, Scalar(0, 255, 0), 1, CV_AA);
    line (m_matResult, ptLB, ptLB - ptDis1 / 2, Scalar(0, 255, 0), 1, CV_AA);
    line (m_matResult, ptLB, ptLB + ptDis2 / 2, Scalar(0, 255, 0), 1, CV_AA);
    //

    DrawDashLine (m_matResult, ptLT + ptDis1, ptLT + ptDis2);
    DrawMarkCross (m_matResult, ptC.x, ptC.y, 5, Scalar(0, 255, 0), 1);
    string str = format ("%d", i);
    putText (m_matResult, str, (ptLT + ptRT) / 2, FONT_HERSHEY_PLAIN, 1, Scalar(0, 255, 0));
  }
